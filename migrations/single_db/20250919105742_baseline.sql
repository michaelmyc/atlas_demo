...
CREATE TABLE `categories` (
  ...
  CONSTRAINT `categories_ibfk_1` FOREIGN KEY (`parent_category_id`) REFERENCES `categories` (`category_id`) ON UPDATE CASCADE ON DELETE CASCADE,
...
CREATE TABLE `users` (
  ...
  CONSTRAINT `users_ibfk_1` FOREIGN KEY (`parent_user_id`) REFERENCES `users` (`user_id`) ON UPDATE CASCADE ON DELETE SET NULL
) CHARSET utf8mb4 COLLATE utf8mb4_unicode_ci;
-- Create "orders" table
CREATE TABLE `orders` (
  ...
  CONSTRAINT `orders_ibfk_1` FOREIGN KEY (`user_id`) REFERENCES `users` (`user_id`) ON UPDATE CASCADE ON DELETE CASCADE,
...
CREATE TRIGGER `update_stock_on_order_item_update` AFTER UPDATE ON `order_items` FOR EACH ROW BEGIN
...
CREATE TABLE `audit_log` (
  ...
  CONSTRAINT `audit_log_ibfk_1` FOREIGN KEY (`changed_by_user_id`) REFERENCES `users` (`user_id`) ON UPDATE CASCADE ON DELETE SET NULL
) CHARSET utf8mb4 COLLATE utf8mb4_unicode_ci;
...
CREATE TABLE `reviews` (
  ...
  CONSTRAINT `reviews_ibfk_1` FOREIGN KEY (`product_id`) REFERENCES `products` (`product_id`) ON UPDATE CASCADE ON DELETE CASCADE,
  CONSTRAINT `reviews_ibfk_2` FOREIGN KEY (`user_id`) REFERENCES `users` (`user_id`) ON UPDATE CASCADE ON DELETE CASCADE,
...
CREATE VIEW `order_summary` (
  ...
  `item_count`,
  `calculated_total`
) AS select `o`.`order_id` AS `order_id`,`o`.`user_id` AS `user_id`,`u`.`username` AS `username`,`o`.`total_amount` AS `total_amount`,`o`.`status` AS `status`,count(`oi`.`order_item_id`) AS `item_count`,sum(`oi`.`subtotal`) AS `calculated_total` from ((`orders` `o` join `users` `u` on((`o`.`user_id` = `u`.`user_id`))) join `order_items` `oi` on((`o`.`order_id` = `oi`.`order_id`))) group by `o`.`order_id`,`o`.`user_id`,`u`.`username`,`o`.`total_amount`,`o`.`status` having (`calculated_total` = `o`.`total_amount`);
